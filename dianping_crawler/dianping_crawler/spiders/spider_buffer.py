...
class SpiderBufferSpider(scrapy.Spider):
    name = "spider_buffer"
    ...
    def __init__(self):
        self.start_urls = ['http://www.dianping.com/search/category/3/10/g101r7947p2']
    ...
    def parse(self, response):
        try:
            with open(self.spider_path + 'crawled_urls', 'a+') as fw:
                fw.write(response.url + "\n")
        except Exception as e:
            self.log("crawled_urls error: " + str(e), level=log.ERROR)
            print e
        # self.delay(response.url)
        print chardet.detect(response.body)
        shop_items = []
        shop_urls = []
        shop_items, shop_urls = self.get_init_data()
        try:
            with open(self.spider_path + "shop_urls.txt", 'a+') as fw:
                fw.write("\n".join(shop_urls) + "\n")
        except Exception as e:
